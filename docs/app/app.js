...
app.innerHTML = `
  ...
  function renderOpponentSearch(query) {
    ...
    if (!q) {
      ...
      const maxTogether = new Map();

      /*
       * 現在選択している相手ポケモン。
       * フォーム差は無視しspecies_id単位で扱う。
       */
      ...
      const collectMatch = match => {
        ...
        teamSpecies.forEach(speciesId => {
          ...
          if (selectedSpecies.size === 0) {
            scores.set(
              speciesId,
              (scores.get(speciesId) || 0) + 1
            );

            appearances.set(
              speciesId,
              (appearances.get(speciesId) || 0) + 1
            );
      ...
      groups =
        allGroups
          .filter(group =>
            scores.has(group.species_id)
          )
          .sort((a, b) => {
            const aId = a.species_id;
            const bId = b.species_id;
             ...
             */
            const togetherDiff =
              (maxTogether.get(bId) || 0) -
              (maxTogether.get(aId) || 0);
            ...
            const appearanceDiff =
              (appearances.get(bId) || 0) -
              (appearances.get(aId) || 0);
      ...
      if (groups.length) {
        ...
        if (selectedSpecies.size === 0) {
          ...
          label.textContent =
            `選択中の${selectedSpecies.size}匹との共起候補`;
        }
